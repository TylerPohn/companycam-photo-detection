"""YOLOv8-based damage detection model wrapper (Mock Implementation)"""
...
from src.schemas.damage_detection import DamageType, BoundingBox
...
class DamageDetector:
    """
    YOLOv8-based damage detector for roof damage.
    ...
    def __init__(self, config: Optional[DetectorConfig] = None):
        ...
        logger.info(f"Initializing DamageDetector with config: {self.config.model_dump()}")
    ...
    def load_model(self):
        ...
        logger.info("YOLOv8 model loaded successfully (MOCK)")
    ...
    def detect(self, image: Image.Image) -> List[DetectionResult]:
        ...
        Args:
            image: PIL Image to detect damage in
        ...
        logger.debug(
            f"YOLOv8 detection completed: {len(final_detections)} detections "
    ...
    def _generate_mock_detections(self, img_width: int, img_height: int) -> List[DetectionResult]:
        ...
        # Randomly generate 0-5 damage detections
        ...
        for _ in range(num_damages):
            ...
            # Random bounding box
            box_width = random.randint(50, min(300, img_width // 2))
            box_height = random.randint(50, min(300, img_height // 2))
